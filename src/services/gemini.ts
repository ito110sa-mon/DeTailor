...
- 冗長な表現を避け、すっきりと整理されたレイアウトにしてください。`,
  ...
  blog: `他者が伝えることを意識した、構成の整った「ブログ風」の記事として仕立ててください。
...
export interface TailoredDiary {
  title: string;
  content: string;
}
...
"""
【優先ルール】:
上記の「ユーザーからの詳細な仕立てオーダー」に記載された指示は、選択されたスタイル（${STYLE_LABELS[style]}）の基本ルールよりも優先して適用してください。例えば、感情表現の方向性、特定の言葉遣い、文字数の制限、特定のトーンや構成指定などがある場合、必ずそれに従って仕立ててください。
`;
  }
...
`;
  ...
  try {
    ...
    return {
      title: parsedTitle || title.trim() || "無題の日記",
